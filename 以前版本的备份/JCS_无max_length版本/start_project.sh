...
start_lightrag() {
    ...
    (
        cd /data/LightRAG || { log_error "无法进入 LightRAG 目录"; exit 1; }
        source .venv310/bin/activate || { log_error "无法激活 LightRAG 虚拟环境"; exit 1; }
        cd /data/LightRAG/examples/ || { log_error "无法进入 examples 目录"; exit 1; }
        echo "========== LightRAG 启动输出 =========="
        python lightrag_bmodel2.py || { log_error "lightrag_bmodel2.py 启动失败"; exit 1; }
        echo "========== LightRAG 结束输出 =========="
    ) &
    
    LIGHTRAG_PID=$!
    echo $LIGHTRAG_PID > /tmp/lightrag.pid || { log_error "无法保存 LightRAG PID"; exit 1; }
    
    # 检查进程是否成功启动
    sleep 2
    if ! kill -0 $LIGHTRAG_PID 2>/dev/null; then
        log_error "lightrag_bmodel2.py 启动后立即退出，检查日志"
        exit 1
    fi
...
cleanup() {
    ...
    if [ -f "/tmp/lightrag.pid" ]; then
        ...
        if kill -0 $LIGHTRAG_PID 2>/dev/null; then
            ...
            # 如果进程仍在运行，强制杀死
            if kill -0 $LIGHTRAG_PID 2>/dev/null; then
                log_warn "强制终止 lightrag_bmodel2.py"
                kill -KILL $LIGHTRAG_PID 2>/dev/null || true
            fi
        fi
    ...
    # 终止所有相关的Python进程
    log_info "终止所有相关进程..."
    pkill -f "sample_audio.py" 2>/dev/null || true
    pkill -f "lightrag_bmodel2.py" 2>/dev/null || true
...
main() {
    case "${1:-start}" in
        "start")
            ...
            cd /data/whisper-TPU_py || { log_error "无法进入 whisper-TPU_py 目录"; exit 1; }
            source .venv/bin/activate || { log_error "无法激活 whisper 虚拟环境"; exit 1; }
            cd bmwhisper/ || { log_error "无法进入 bmwhisper 目录"; exit 1; }
            ...
            python3 sample_audio.py || { log_error "sample_audio.py 执行失败"; cleanup; exit 1; }
