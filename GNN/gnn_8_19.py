...
from torch_geometric.nn import GCNConv, Set2Set # <-- 修改为 GCNConv
from torch_geometric.utils import to_networkx, degree

# 句子嵌入模型
from sentence_transformers import SentenceTransformer, util # <-- 导入 util 用于计算相似度
...
class MermaidDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        # 使用新文件名以避免与旧缓存冲突
        return ['processed_data_semantic_graph.pt']
    ...
    def process(self):
        ...
        for index, row in tqdm(self.raw_dataframe.iterrows(), total=self.raw_dataframe.shape[0]):
            try:
                ...
                for i in range(num_nodes):
                    for j in range(i + 1, num_nodes): # 遍历所有节点对
                ...
                if num_nodes > 0:
                    # 使用原有的显式边来计算结构特征
                    if edges:
                        valid_edges_struct = [e for e in edges if e[0] in node_id_to_idx and e[1] in node_id_to_idx]
                        if valid_edges_struct:
                            src_struct = [node_id_to_idx[s] for s, t in valid_edges_struct]
                            tgt_struct = [node_id_to_idx[t] for s, t in valid_edges_struct]
                            edge_index_struct = torch.tensor([src_struct, tgt_struct], dtype=torch.long)
                        else:
                            edge_index_struct = torch.empty((2, 0), dtype=torch.long)
                    else:
                        edge_index_struct = torch.empty((2, 0), dtype=torch.long)
                    
                    g_nx_struct = to_networkx(Data(edge_index=edge_index_struct, num_nodes=num_nodes), to_undirected=False)
                    ...
                    if not root_nodes and num_nodes > 0: root_nodes = [0]
                    ...
                    betweenness = torch.tensor(list(nx.betweenness_centrality(g_nx_struct).values()), dtype=torch.float).view(-1, 1)
                    closeness = torch.tensor(list(nx.closeness_centrality(g_nx_struct).values()), dtype=torch.float).view(-1, 1)

                    node_features = torch.cat([
                        in_degree / num_nodes, out_degree / num_nodes, node_depths / num_nodes, betweenness, closeness
                    ], dim=1)
                else:
                    node_features = torch.empty((0, 5), dtype=torch.float)
                ...
                graph_data = Data(x=node_features.float(), edge_index=edge_index, edge_attr=edge_attr, y=y)
                
                # --- 全局图级别特征 (使用显式结构图计算) ---
                num_edges_struct = g_nx_struct.number_of_edges()
                ...
                degree_centrality = list(nx.degree_centrality(g_nx_struct).values())
                degree_std = np.std(degree_centrality) if degree_centrality else 0.0
                
                num_leaves = sum(1 for _, d in g_nx_struct.out_degree() if d == 0)
                ...
                graph_data.graph_features = torch.tensor([graph_level_features_list], dtype=torch.float)
        ...
        # --- 【新增】在 collate 之前检查 data_list 是否为空 ---
        if not data_list:
            ...
            raise RuntimeError("处理后没有任何有效的数据！"
                               "请检查您的CSV文件中的Mermaid代码是否存在普遍的格式问题，"
                               "或者 parse_mermaid 函数是否足够健壮。")
        
        data, slices = self.collate(data_list)
        torch.save((data, slices), self.processed_paths[0])

# ==============================================================================
# 2. GNN模型定义 (已修改为GCNScorer)
# ==============================================================================

class GCNScorer(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_graph_features=7):
        ...
        mlp_input_dim = 2 * (hidden_dim * 2) + num_graph_features
    ...
    def forward(self, data):
        # 从data对象中获取 x, edge_index, batch, edge_attr, 和 graph_features
        x, edge_index, batch, edge_attr, graph_features = \
            data.x, data.edge_index, data.batch, data.edge_attr, data.graph_features
        
        # 将 edge_attr 用作 GCN 的 edge_weight
        ...
        final_embedding = torch.cat([graph_embedding_pooled, graph_features], dim=1)
...
if __name__ == '__main__':
    ...
    dataset = MermaidDataset(root='./data_processed_semantic_graph', raw_dataframe=raw_dataframe)
    ...
    print(f"最终的节点特征维度: {input_dim}") # 应该是 5
    
    model = GCNScorer(input_dim=input_dim, hidden_dim=128, num_graph_features=7).to(device)
    ...
    loss_weights = [1.0, 1.0, 1.0, 1.0, 5.0]  # [结构, 内容, 层次, 语法, 总分]
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_model_semantic_graph.pt') # <-- 使用新模型名
            early_stop_counter = 0
        else:
    ...
    # --- 在测试集上评估最终模型 ---
    print("\n--- 在测试集上进行最终评估 ---")
    model.load_state_dict(torch.load('best_model_semantic_graph.pt'))
