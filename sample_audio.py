...
class QwenAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/generate':
            # 在进入锁之前，先设置高优先级标志，通知其他线程让路
            globalConfig.rag_task_active = True
            print("\n[INFO] High-priority RAG request received. Acquiring LLM lock...", flush=True)
            
            
            # 使用 try...finally 确保标志位一定会被重置
            try:
                # 获取LLM锁，如果思维导图线程正在使用，会等待其释放
                with llm_lock:
                    print("[INFO] LLM lock acquired for RAG task.", flush=True)   
                    # ---- 从这里开始是原有的流式处理代码 ---- 
                    try:
                        # 读取请求数据
                ...
                # 任务完成，重置高优先级标志，并释放锁（with语句会自动完成）
                globalConfig.rag_task_active = False
                print("[INFO] RAG task finished. Releasing LLM lock and priority flag.", flush=True)       
    ...
    while True:
        ...
        try:
            ...
            if now - last_qwen_time >= globalConfig.qwen_interval:
                ...
                # 新增：在尝试生成前，检查是否有高优先级任务
                if globalConfig.rag_task_active:
                    print("[INFO] RAG task is active, skipping Mermaid generation for now.", flush=True)
                    # 不重置 last_qwen_time，以便下次循环立即重试
                    continue
                
                # 使用锁来安全地调用LLM
                print("[INFO] Mermaid thread attempting to acquire LLM lock...", flush=True)
                with llm_lock:
                    # 再次检查，防止在等待锁的过程中，RAG任务插队
                    if globalConfig.rag_task_active:
                        print("[INFO] RAG task took priority while waiting. Skipping.", flush=True)
                        continue
                    
                    print("[INFO] LLM lock acquired for Mermaid generation.", flush=True)
                ...
                print("[INFO] Mermaid generation finished, lock released.", flush=True)
